...
const props = defineProps({
    course: {
        type: Object as PropType<EndpointResponseBody<"courses/:id"> | null>,
        default: () => null,
    },
});
            ...
            <div v-else class="lg:flex">
                <div class="w-full lg:inline-block lg:w-2/3 lg:pr-1">
                    <h2 title="授課系所與教師" class="my-1 text-sm text-gray-600">
                        {{ course.provider }} {{ course.teachers.join("、") }}
                    </h2>
                    ...
                    <h1 class="my-1 text-xl font-bold">
                        ...
                        <span class="text-base">
                            ...
                            <span class="text-gray-400"> | </span>
                            <span title="該期開課序號" class="text-gray-600">{{ course.code }}</span>
                            <span class="text-gray-400"> | </span>
                    ...
                    <h2 title="學分與授課時段" class="my-1 text-sm text-gray-500">
                        ...
                        <!-- ，實際授課
                        <span
                            :class="['font-bold', course.extra.hours > course.credit ? 'text-yellow-500' : 'text-green-500']">{{
                                    course.hours
                            }}</span>
                        小時 -->
                        <br class="sm:hidden" />
                        <!-- ({{ course.extra.syllabus.map(readable_schedule).join("、") }}) -->
                    </h2>
                    ...
                    <div class="my-3">
                        ...
                        <AniFade direction="up">
                            <div v-show="!fold.choose" class="overflow-hidden p-2 text-gray-700">
                                <p>
                                    名額： <span class="font-bold text-indigo-600">{{ course.extra.quota.limit }}</span> 人 + 授權碼
                                    <span class="font-bold text-indigo-600">x{{ course.extra.quota.additional }}</span>
                                </p>
                                <p v-if="course.extra.restrict" class="text-red-600">限制： {{ course.extra.restrict }}</p>
                                <p v-if="course.extra.comment">備註： {{ course.extra.comment }}</p>
                                <p v-if="!course.extra.restrict && !course.extra.comment">沒有其餘限制</p>
                            </div>
                        </AniFade>
                    </div>
                    ...
                    <div class="my-3">
                        ...
                        <AniFade direction="up">
                            <div v-show="!fold.goals" class="overflow-hidden p-2">
                                <div v-for="(goal, idx) of course.extra.goals" :key="idx" class="my-1">
                    ...
                    <div class="my-3">
                        ...
                        <hr class="mt-1 mb-2" />
                        <AniFade direction="up">
                            <div v-show="!fold.syllabus" class="overflow-hidden p-2" v-html="marked.parse(course.extra.syllabus)"></div>
                        </AniFade>
                    </div>
                </div>
